...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        // get the boot block address
        boot_block_address = mod->mod_start;
    ...
    filesystem_init(boot_block_address);
...
#ifdef RUN_TESTS
    /* Run tests */
    // uncomment the following instruction to enable testing
    launch_tests();
#endif
